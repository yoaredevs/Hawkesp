...
local function teamAllowed(player)
    local any = true
    for _ in pairs(AB.Teams) do any = false; break end
    if any then return true end
    return AB.Teams[teamNameOf(player)] == true
end
...
isValidTarget = function(player)
    if not teamAllowed(player) then return false end
...
local function applyTeams(selection)
    ...
    if type(selection) == "table" then
        for k, v in pairs(selection) do
            if v == true then AB.Teams[tostring(k)] = true
            elseif type(v) == "string" then AB.Teams[v] = true end
        end
    elseif type(selection) == "string" then
        AB.Teams[selection] = true
    end
...
-- Auto Fire
--------------------------------------------------------------
TabAim:Section({ Title = "Auto Fire" })
TabAim:Toggle({
    Title = "Auto Fire",
    Desc = "Atira enquanto está travado (deixe o 'Auto Shoot' do bloco antigo OFF)",
    Value = false,
    Callback = function(v)
        AB.AutoFire = v
