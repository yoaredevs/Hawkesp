...
TabMisc:Button({
    ...
    Callback = function()
        if Want.Active then
            applyNow()
            WindUI:Notify({ Title = "Hawk", Content = "Sky re-applied.", Duration = 2, Icon = "refresh-cw" })
        else
            WindUI:Notify({ Title = "Hawk", Content = "No custom sky active.", Duration = 2, Icon = "triangle-alert" })
        end
    end,
})
TabMisc:Button({
    Title = "Restore Original Sky",
    Desc = "Puts the game's own lighting / sky back",
    Callback = function()
        restoreSky()
        WindUI:Notify({ Title = "Hawk", Content = "Sky restored.", Duration = 2, Icon = "undo-2" })
    end,
...
table.insert(extraConns, {
    Disconnect = function() pcall(restoreSky) end,
})
end
...
-- KEYBINDS / UNLOAD
--==============================================================
TabMisc:Section({ Title = "Keybinds" })

local UIToggleKey = Enum.KeyCode.RightControl
local function setWindowOpen(state)
    UIState.Open = state
    pcall(function()
        if state then Window:Open() else Window:Close() end
    end)
end

TabMisc:Keybind({
    Title = "Toggle UI",
    Desc = "Shows / hides the Hawk window",
    Value = "RightControl",
    Callback = function(key)
        local kc = Enum.KeyCode[key]
        if kc then UIToggleKey = kc end
    end,
})
...
table.insert(extraConns, UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == UIToggleKey then
        local isOpen = (Window.Closed ~= true) and UIState.Open
        setWindowOpen(not isOpen)
    end
end))

TabMisc:Section({ Title = "Unload" })
TabMisc:Button({
    Title = "Unload Hawk ESP",
    Desc = "Removes every drawing, connection and the window",
    Callback = function()
        pcall(unload)
        pcall(function() Window:Destroy() end)
        pcall(function()
            StarterGui:SetCore("SendNotification", { Title = "Hawk ESP", Text = "Unloaded.", Duration = 3 })
        end)
    end,
})
...
WindUI:Notify({
    Title = "Hawk ESP",
    Content = IsPrisonLife and "Loaded — Prison Life features enabled." or "Loaded — generic mode (not Prison Life).",
    Duration = 5,
    Icon = "eye",
} )