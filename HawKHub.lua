...
renderConn = RunService.RenderStepped:Connect(function(dt)
    ...
    -- Aim FOV circle
    if AIM.Enabled and AIM.UseFOV then
        local vp = Camera.ViewportSize
...
--==============================================================
TabAim:Section({ Title = "Mobile Aimbot" })

local MOBILE = {
    ButtonEnabled = false,
    ButtonSize    = 78,
    ButtonOpacity = 0.35,
    AutoTouch     = false,   -- aim while any finger is held on screen
    GunOnly       = true,    -- only aim while holding a weapon
    ArrestedCheck = true,    -- ignore already-arrested players
}
...
local function plIsArrested(player)
    ...
    if not ch then return false end
    -- PL cuffs the player and forces walkspeed 0 with a "Handcuffs" model welded
...
local function buildMobileButton()
    ...
    MobileGui.Name = "HawkMobileAim"
    MobileGui.ResetOnSpawn = false
    MobileGui.IgnoreGuiInset = true
    MobileGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    MobileGui.DisplayOrder = 999
    pcall(function() MobileGui.Parent = gethui() end)
    if not MobileGui.Parent then MobileGui.Parent = LocalPlayer:WaitForChild("PlayerGui") end

    MobileBtn = Instance.new("TextButton")
    MobileBtn.Name = "AimButton"
    MobileBtn.Size = UDim2.fromOffset(MOBILE.ButtonSize, MOBILE.ButtonSize)
    MobileBtn.Position = UDim2.new(0.78, 0, 0.62, 0)
    MobileBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
    MobileBtn.BackgroundTransparency = MOBILE.ButtonOpacity
    MobileBtn.Text = "AIM"
    MobileBtn.TextColor3 = Color3.fromRGB(0, 255, 170)
    MobileBtn.TextSize = 16
    MobileBtn.Font = Enum.Font.GothamBold
    MobileBtn.AutoButtonColor = false
    MobileBtn.Active = true
    MobileBtn.Draggable = false
    ...
    -- drag with a second finger / long move, aim with a tap-hold
    ...
    MobileBtn.InputBegan:Connect(function(input)
        ...
        MobileBtn.BackgroundTransparency = math.max(0, MOBILE.ButtonOpacity - 0.25)
    ...
    local function release()
        if not dragging then return end
        dragging = false
        if AIM.Enabled and AIM.HoldMode then
            AimState.Active = false
            AimState.Target = nil
        end
        MobileBtn.BackgroundTransparency = MOBILE.ButtonOpacity
    ...
    MobileBtn.InputEnded:Connect(release)
    MobileBtn.MouseLeave:Connect(function() if not moved then release() end end)
end

TabAim:Toggle({
    Title = "Mobile Aim Button",
    Desc = isMobile and "Hold the button to lock. Drag it to reposition."
                     or "For touch devices — hold to lock, drag to move",
    Value = false,
    Callback = function(v)
        MOBILE.ButtonEnabled = v
        if v then buildMobileButton() else destroyMobileButton() end
    end,
})
TabAim:Slider({
    Title = "Button Size", Step = 2,
    Value = { Min = 50, Max = 140, Default = 78 },
    Callback = function(v)
        MOBILE.ButtonSize = v
        if MobileBtn then MobileBtn.Size = UDim2.fromOffset(v, v) end
    end,
})
TabAim:Slider({
    Title = "Button Transparency", Step = 0.05,
    Value = { Min = 0, Max = 0.9, Default = 0.35 },
    Callback = function(v)
        MOBILE.ButtonOpacity = v
        if MobileBtn then MobileBtn.BackgroundTransparency = v end
    end,
})
TabAim:Toggle({
    Title = "Aim While Shooting (touch)",
    Desc = "Locks automatically while any finger is on the screen",
    Value = false,
    Callback = function(v) MOBILE.AutoTouch = v end,
})
TabAim:Toggle({
...
-- touch-hold aim + weapon gate
local touchCount = 0
table.insert(extraConns, UserInputService.TouchStarted:Connect(function()
    touchCount = touchCount + 1
    if MOBILE.AutoTouch and AIM.Enabled then
        AimState.Active = true
        AimState.Target = getTarget()
    end
end))
table.insert(extraConns, UserInputService.TouchEnded:Connect(function()
    touchCount = math.max(0, touchCount - 1)
    if MOBILE.AutoTouch and touchCount == 0 then
        AimState.Active = false
        AimState.Target = nil
    end
end))
table.insert(extraConns, RunService.RenderStepped:Connect(function()
    if MOBILE.GunOnly and AimState.Active and not plHoldingGun() then
        AimState.Active = false
        AimState.Target = nil
    end
end))
...
-- clean the mobile button up on unload
local _plOldUnload = unload
unload = function()
...
-- keep a snapshot of the original sky/atmosphere so we can restore
local SkyBackup = {}
...
local function getSky()
    if HawkSky and HawkSky.Parent then return HawkSky end
    -- hide the game's original skies
    for _, obj in ipairs(SkyBackup) do
        if obj:IsA("Sky") then pcall(function() obj.Parent = nil end) end
    end
...
local function getAtmo()
    if HawkAtmo and HawkAtmo.Parent then return HawkAtmo end
    HawkAtmo = Lighting:FindFirstChildOfClass("Atmosphere")
    if not HawkAtmo then
...
-- id, id, id, id, id, id = Up, Lf, Rt, Bk, Ft, Dn
local SkyPresets = {
...
local function restoreSky()
    ...
    if HawkAtmo and HawkAtmo.Name == "HawkAtmosphere" then
        pcall(function() HawkAtmo:Destroy() end)
        HawkAtmo = nil
    end
    for _, obj in ipairs(SkyBackup) do
...
TabMisc:Dropdown({
    Title = "Skybox Preset",
    Desc = "Changes the sky texture in any game",
    ...
    Callback = function(v)
        ...
        local sky = getSky()
        sky.SkyboxUp, sky.SkyboxDn = id, id
        sky.SkyboxLf, sky.SkyboxRt = id, id
        sky.SkyboxFt, sky.SkyboxBk = id, id
        sky.StarCount = 3000
...
TabMisc:Input({
    ...
    Desc = "Paste a decal/asset id and press Enter (applies to all 6 faces)",
    ...
    Callback = function(txt)
        ...
        local sky = getSky()
        sky.SkyboxUp, sky.SkyboxDn = asset, asset
        sky.SkyboxLf, sky.SkyboxRt = asset, asset
        sky.SkyboxFt, sky.SkyboxBk = asset, asset
...
TabMisc:Toggle({
    Title = "Show Sun / Moon",
    Value = true,
    Callback = function(v) getSky().CelestialBodiesShown = v end,
})
TabMisc:Slider({
    Title = "Star Count", Step = 500,
    Value = { Min = 0, Max = 8000, Default = 3000 },
    Callback = function(v) getSky().StarCount = v end,
})

TabMisc:Section({ Title = "Lighting" })
TabMisc:Slider({
    Title = "Time of Day", Step = 0.1,
    Desc = "0 = midnight, 12 = noon",
    ...
    Callback = function(v)
        pcall(function()
            if v then
                ...
                Lighting.Brightness      = SkyOriginal.Brightness
                Lighting.GlobalShadows   = SkyOriginal.GlobalShadows
                Lighting.Ambient         = SkyOriginal.Ambient
                Lighting.OutdoorAmbient  = SkyOriginal.OutdoorAmbient
                Lighting.EnvironmentDiffuseScale  = SkyOriginal.EnvironmentDiffuseScale
                Lighting.EnvironmentSpecularScale = SkyOriginal.EnvironmentSpecularScale
            end
        end)
    end,
})
    ...
    Callback = function(v)
        pcall(function()
            if v then
                ...
                if a then a.Density = 0; a.Haze = 0; a.Glare = 0 end
            else
...
TabMisc:Colorpicker({
    ...
    Callback = function(c) pcall(function() Lighting.FogColor = c end) end,
})
TabMisc:Slider({
    Title = "Atmosphere Density", Step = 0.01,
    Value = { Min = 0, Max = 1, Default = 0.3 },
    Callback = function(v) getAtmo().Density = v end,
})
TabMisc:Slider({
    Title = "Atmosphere Haze", Step = 0.1,
    Value = { Min = 0, Max = 10, Default = 0 },
    Callback = function(v) getAtmo().Haze = v end,
})
TabMisc:Slider({
    Title = "Atmosphere Glare", Step = 0.1,
    Value = { Min = 0, Max = 10, Default = 0 },
    Callback = function(v) getAtmo().Glare = v end,
})
TabMisc:Colorpicker({
    Title = "Atmosphere Color",
    Default = Color3.fromRGB(199, 170, 107),
    Callback = function(c) getAtmo().Color = c end,
})
TabMisc:Toggle({
    Title = "Remove Clouds",
    Value = false,
...
local function applyMood(mood)
    ...
    if mood == "Midnight" then
        local id = SkyPresets["Night / Stars"]
        for _, f in ipairs({"SkyboxUp","SkyboxDn","SkyboxLf","SkyboxRt","SkyboxFt","SkyboxBk"}) do sky[f] = id end
        Lighting.ClockTime = 0
        Lighting.Brightness = 1
        Lighting.OutdoorAmbient = Color3.fromRGB(40, 45, 70)
        atmo.Density = 0.35; atmo.Haze = 1; atmo.Color = Color3.fromRGB(60, 70, 120)
    elseif mood == "Golden Sunset" then
        local id = SkyPresets["Sunset"]
        for _, f in ipairs({"SkyboxUp","SkyboxDn","SkyboxLf","SkyboxRt","SkyboxFt","SkyboxBk"}) do sky[f] = id end
        Lighting.ClockTime = 17.5
        Lighting.Brightness = 2.5
        Lighting.OutdoorAmbient = Color3.fromRGB(120, 90, 70)
        atmo.Density = 0.4; atmo.Haze = 3; atmo.Glare = 1
        atmo.Color = Color3.fromRGB(220, 160, 90)
    elseif mood == "Deep Space" then
        local id = SkyPresets["Space"]
        for _, f in ipairs({"SkyboxUp","SkyboxDn","SkyboxLf","SkyboxRt","SkyboxFt","SkyboxBk"}) do sky[f] = id end
    ...
    elseif mood == "Toxic Green" then
        ...
        Lighting.FogColor = Color3.fromRGB(60, 120, 60)
        Lighting.OutdoorAmbient = Color3.fromRGB(80, 140, 80)
        atmo.Density = 0.5; atmo.Haze = 5
        atmo.Color = Color3.fromRGB(90, 200, 90)
    end
...
TabMisc:Button({
    ...
    Desc = "Puts the game's own sky and lighting back",
    Callback = function()
        restoreSky()
