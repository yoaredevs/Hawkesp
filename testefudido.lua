...
--==============================================================
-- COMPAT WINDUI
-- Versoes diferentes do WindUI expoem metodos diferentes. Chamar
-- direto dentro de pcall escondia a falha: o dropdown simplesmente
-- nunca atualizava e nada era avisado. Aqui tentamos os nomes
-- conhecidos e devolvemos se algum funcionou.
--==============================================================
local HawkUI = {}

function HawkUI.refresh(component, values, label)
    if component == nil then return false end
    local okType = pcall(function() return component.Refresh end)
    if not okType then return false end
    for _, name in ipairs({ "Refresh", "SetValues", "Reload", "UpdateValues", "SetOptions" }) do
        local fn
        pcall(function() fn = component[name] end)
        if type(fn) == "function" then
            local ok = pcall(fn, component, values)
    ...
    if label then
        pcall(function()
            WindUI:Notify({
                Title = "Hawk",
                Content = "Esta versao do WindUI nao permite atualizar a lista: " .. label,
                Duration = 4,
                Icon = "triangle-alert",
            })
        end)
...
function HawkUI.setTheme(name)
    local fn
    pcall(function() fn = WindUI.SetTheme end)
    if type(fn) ~= "function" then
        pcall(function()
            WindUI:Notify({
                Title = "Hawk",
                Content = "WindUI carregado sem SetTheme — tema mantido no padrao.",
                Duration = 4,
                Icon = "triangle-alert",
            })
        end)
    ...
    return (pcall(fn, WindUI, name))
end

HawkUI.setTheme("Hawk")
...
TabAim:Button({
    ...
    Callback = function()
        ...
        HawkUI.refresh(TeamDropdown, names, "Times Alvo")
...
TabTP:Button({
    ...
    Callback = function()
        ...
        HawkUI.refresh(PlaceDropdown, names, "Map Locations")
...
TabTP:Button({
    ...
    Callback = function()
        Givers = getGivers()
        HawkUI.refresh(GunDropdown, giverNames(), "Guns / Items")
...
TabTP:Button({
    ...
    Callback = function() HawkUI.refresh(PlayerDropdown, playerNames(), "Teleport to Player") end,
})
TabTP:Toggle({
...
TabTP:Button({
    ...
    Callback = function()
        ...
        HawkUI.refresh(WaypointDropdown, waypointNames(), "Waypoints")
